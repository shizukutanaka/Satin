...
| S2 | **研究駆動の安全設計**: 感情依存ガードレール・否定/強度対応感情判定・BM25 記憶想起・変化点検知・概日トーン・**別れぎわの引き止め文句ゼロ**・**危機表明への相談先案内**・**AI であることの定期開示** | `usage_guardrails.py`, `mood.py`, `conversation_log.search_relevant`, `user_wellbeing.wellbeing_shift`, `persona.py`, `farewell_integrity.py`, `crisis_support.py`, `ai_disclosure.py`。詳細は `RESEARCH_DRIVEN_IMPROVEMENTS_PY.md` |
