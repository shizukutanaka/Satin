...
| **プライバシーローカル主義** | 会話・好感度・プロフィールは全てローカル保存。ディスク上は所有者のみ読み書き可 (0600) — **エクスポート・アーカイブ・ダッシュボードの zip を含む**（`fsutil.atomic_write_text(restrict=True)` / `restrict_to_owner`）。`data purge` で完全消去可能。`conversation_retention_days` で保存期間の上限も設定可（既定 0 = 無期限）。 |
...
- **上限は「稼げる上昇」すべてで共有される**（`MoodTracker.earn()`）: 会話・
  プレゼント・おやすみ/謝罪のルーティンボーナス・デイリーログインボーナス。
  予算を分けると上限が弧の長さを決めなくなる。実際に迂回していた経路が 4 つ
  あった — プレゼントは 7 種を配るだけで初日に最高レベルへ到達でき（合計 30.5 が
  ちょうど 50.0→80.0 を埋める）、「おやすみ」を 20 回打つと初日に affinity 90、
  ログインボーナス（2.0〜5.0/日）は上限の外に積まれて「最短 6 日」が実際には
  4 日目に終わっていた。CLI だけが会話にデイリームード乗数を上限の外で掛けて
  いた（GUI には無い）が、予算を通せば予算を使い切る速さしか変わらないので
  削除し、GUI と同じ挙動にした。上限に達した状態で贈っても好感度の数字は
  表示しない（反映されていない値を見せないため）。
  `adjust()` で上限の外に置くのは、誕生日（年 1 回）・記念日（5 回きり）のように
  **繰り返せない**ボーナスだけ。上限は上昇のみに掛かり、ネガティブな発言の
  ペナルティは薄めない。
- **自然低下は成長と対称**: 0.2/時間（≒ 4.8/日、日次上限 5.0 と釣り合う）で、
  最初の 48 時間は低下しない。週末そのぶん離れても関係は冷えず、1 週間の不在で
  約 -25。かつては 2.0/時間・猶予なしで、24 時間で -48・40 時間で最低レベルに
  落ちた — 育つのに 6 日かかるものが 2 日で消える非対称であり、`usage_guardrails`
  が「休んでいいよ」と促した直後に休息を罰していた。
- **デイリーログインボーナスは日次予算の 40% まで**（`_DAILY_LOGIN_BUDGET_SHARE`）。
  上限が無いと連続 7 日目でログインだけで予算を使い切り、その日は何回会話しても
  好感度が動かなかった。会話コンパニオンで「話すこと」より「来ること」が報われる
  のは本末転倒であり、ガードレールが警戒するエンゲージメント誘導そのものである。
  連続記録と節目のお祝いメッセージは従来どおり出る（好感度で釣らない）。
...
4. **プライバシー設計**
   ...
   ディスクへ書く経路は 1 つ（`fsutil.atomic_write_text`）に統一されており、
   `restrict=True` で 0600 を付ける。エクスポートは中身が完全な複製なので
   同じ扱いにする — 別々に `open()` していた頃、5 経路が 0644 で書いていた。
5. **原子的書き込みの徹底**
